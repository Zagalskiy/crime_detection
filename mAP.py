...
class MAP:
    ...
    def preprocessData(self):
        ...
        for filename in gtFiles:
            ...
            # anns = np.genfromtxt(os.path.join(self.gtPath, filename), dtype=None, encoding=None)
            anns = anns if anns.ndim > 1 else anns.reshape(1, -1)
            
            if self.ignoreDiff: # Added difficulty flag support
    ...
    def calcmAP(self):
        ...
        IoUList = np.arange(0.5, 1.0, 0.05) if self.cocoFlag else [self.iou]
        classmAPDict = defaultdict(dict)
        APList = [[] for _ in range(self.numClasses)]
        F1List = [[] for _ in range(self.numClasses)]
        
        for iou in IoUList:
            for i in tqdm(range(self.numClasses)):
                dataAP = self.calAPClass(self.gtBoxes[i],
                                         self.detBoxes[i],
                                         self.detScores[i],
                                         IoU=iou)
                classmAPDict[self.classes[i]][iou] = dataAP
                APList[i].append(dataAP['AP'])
                F1List[i].append(dataAP['F1score'])
        
        mAPList = np.array([np.mean(np.array(APList[i])) for i in range(self.numClasses)])
        # print(classmAPDict[cls].values())
        # Print the statistics of the AP@0.5
        table = PrettyTable(['Class/IoU@0.5', 'AP', 'TP', 'FP', 'FN', 'Precision', 'Recall', 'F1 Score', 'GTs', 'Dets'])
        ...
        for cls in classmAPDict.keys():
            table.add_row([
                ...
                '%.3f' % classmAPDict[cls][0.5]['F1score'],
                '%d' % classmAPDict[cls][0.5]['totalGTs'],
                '%d' % classmAPDict[cls][0.5]['totalDets']
            ])
        ...
        if self.cocoFlag:
            ...
            valueTable, f1_list, mAP50_list = [], [], []
            
            for i in range(self.numClasses):
                valueTable.append([self.classes[i]] + ["%.2f" % ap for ap in APList[i]])
                f1_list.append(F1List[i][0])
                mAP50_list.append(APList[i][0])
            
            mf1_score50 = round(np.mean(f1_list), 4)
            mAP50 = round(np.mean(mAP50_list), 4)
            print('mAP@0.5:', mAP50, '\tmean F1 Score:', mf1_score50)
            cocoTable = PrettyTable(headingTable)
            cocoTable.add_rows(valueTable)
            print(cocoTable)
        
        classmAPDict['mAP'] = np.mean(mAPList)
        mAP95 = round(np.mean(mAPList), 4)
        print('mAP@0.5-0.95:', mAP95)
        results = json.dumps(classmAPDict, indent=4)
        
        with open((self.logName + '.json'), 'w') as f:
            f.write(results)
        
        with open((self.logName + '.csv'), 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['mF1_score50', 'mAP50', 'mAP95'])
            writer.writerow([mf1_score50, mAP50, mAP95])
    
    def calAPClass(self, gtBoxes, detBoxes, detScores, IoU=0.5):
        '''
        Input: Ground Truth and Detection Boxes
            Format: [TLX, TLy, BRx, BRy] GT BOXES Dictionary |
                    bbox1 ::: NX4, DETECTION BOXES Dictionary |
                    bbox2 ::: MX4, DETECTION SCORES Dictionary | IoU
        ...
        f1Conf  = np.interp(-self.confidence, -scoresList, f1_score)
    ...
    def getMaskBoxes(self, gtBoxes, detBoxes, IoU): 
        ...
        Input: Ground Truth and Detection Boxes
            Format: [TLX, TLy, BRx, BRy] GT BOXES Dictionary |
                    bbox1 ::: NX4, DETECTION BOXES Dictionary |
                    bbox2 ::: MX4, DETECTION SCORES Dictionary
        Output: Dictionary mask of detections ::: TP - True  / FP - False |
                Key ::: Image Name | Value ::: Mask Array
        Description: Calculate mask of boxes i.e calculate TPs and FPs
        '''
        maskBoxes = {}
    ...
    def getmAPSingle(self, gtBox, detBox, IoU):
        '''
        Input: Ground Truth and Detection Boxes
            Format: [TLX, TLy, BRx, BRy] GT BOXES Shape | 
                    bbox1 ::: NX4, DETECTION BOXES Shape | bbox2 ::: MX4
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--det", type=str, default="detection-results",
                        help="Full path to detection results folder")
    parser.add_argument("-g", "--gt", type=str, default="ground-truths",
                        help="Full path to ground truth folder")
    parser.add_argument("-i", "--iou", type=float, default=0.5,
                        help='Calculate AP at a particular IoU')
    parser.add_argument("-p", "--points", type=int, default=0,
                        help='Interpolation value: 0: Continues / 11: PascalVOC2012 Challenge')
    parser.add_argument("-n", "--names", default="model.names",
                        help="Full path of file containing names of classes index wise")
    parser.add_argument("-o", "--output", default='map_log.json',
                        type=str, help="File to dump the output")
    parser.add_argument("-c", "--confidence", default=0.5, type=float,
                        help="Confidence at which Precision/Recall is calculated")
    parser.add_argument("-ig", "--ignore", action='store_true',
                        help="Flag to ignore the difficult anotations")
    parser.add_argument('--coco', action='store_true',
                        help='COCO Standard of calculation')
